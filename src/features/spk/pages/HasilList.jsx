...
import { Search, RefreshCw, Eye, Trash2, MoreVertical, Calculator } from 'lucide-react'
...
const HasilList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await hasilService.getAll(params)
  ...
  const handleDetail = (data) => navigate(`/spk/hasil/${data.id}`)

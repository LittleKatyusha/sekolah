...
const PenilaianList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await penilaianService.getAll(params)
  ...
  const handleEdit = (data) => navigate(`/spk/penilaian/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/spk/penilaian/${data.id}`)

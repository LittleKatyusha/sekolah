...
const KriteriaList = () => {
  ...
  const handleEdit = useCallback((data) => navigate(`/spk/kriteria/${data.id}/edit`), [navigate])
  const handleDetail = useCallback((data) => navigate(`/spk/kriteria/${data.id}`), [navigate])

  const handleDelete = useCallback(async (data) => {
    const label = `Kriteria "${data.nama_kriteria || ''}"`
    const result = await showDeleteConfirm(label)
    if (result.isConfirmed) {
      const { error } = await kriteriaService.delete(data.id)
      if (!error) {
  ...
  const columnDefs = useMemo(() => [
    ...
    { field: 'kode_kriteria', headerName: 'Kode', sortable: true, filter: true, width: 120, minWidth: 100 },
    { field: 'nama_kriteria', headerName: 'Nama Kriteria', sortable: true, filter: true, flex: 2, minWidth: 200, cellRenderer: (params) => params.value || '-' },
    {
      field: 'bobot',
      headerName: 'Bobot',
      sortable: true,
      ...
      cellRenderer: (params) => (
        <div className="h-full flex items-center justify-center">
          <ActionsMenu
            ...
            canEdit={can('spk-kriteria.update')}
            canDelete={can('spk-kriteria.delete')}
          />
        </div>
      )
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">SPK Kriteria</h1>

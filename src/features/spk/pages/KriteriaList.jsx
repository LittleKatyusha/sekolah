...
const KriteriaList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await kriteriaService.getAll(params)
  ...
  const handleEdit = (data) => navigate(`/spk/kriteria/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/spk/kriteria/${data.id}`)

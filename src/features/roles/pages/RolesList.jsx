...
const RolesList = () => {
  ...
  const fetchRoles = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await roleService.getAll(params)
  ...
  useEffect(() => {
    ...
    fetchRoles(1, pageSize, searchText)
  }, [])

  const handleEdit = (data) => navigate(`/admin/roles/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/admin/roles/${data.id}`)
  ...
  const handleDelete = async (data) => {
    ...
    if (result.isConfirmed) {
      ...
      if (!error) {
        ...
        fetchRoles(currentPageRef.current, pageSize, searchText)
      } else {
  ...
  const onPaginationChanged = useCallback((params) => {
    ...
    if (newPageSize !== pageSize) {
      ...
      pageCursorsRef.current = { 1: null }
      currentPageRef.current = 1
      fetchRoles(1, newPageSize, searchText)
      return
    }
    if (newPageNumber !== currentPageRef.current) {
      fetchRoles(newPageNumber, pageSize, searchText)
    }
  }, [pageSize, searchText, fetchRoles])
  ...
  const onFilterTextBoxChanged = useCallback((e) => {
    ...
    fetchRoles(1, pageSize, value)
  }, [fetchRoles, pageSize])

  const handleRefresh = useCallback(() => {
    fetchRoles(currentPageRef.current, pageSize, searchText)
  }, [fetchRoles, pageSize, searchText])

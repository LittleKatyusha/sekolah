...
const PendaftarList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await pendaftarService.getAll(params)
    if (data) {
  ...
  const handleEdit = (data) => navigate(`/ppdb/pendaftar/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/ppdb/pendaftar/${data.id}`)
  ...
  const onFilterTextBoxChanged = useCallback((e) => {
    ...
    pageCursorsRef.current = { 1: null }
    currentPageRef.current = 1
    if (gridRef.current) gridRef.current.api.paginationGoToPage(0)

...
const TarifSppList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await tarifSppService.getAll(params)
    ...
    if (data) {
      ...
      console.error('Error fetching tarif SPP:', error)
  ...
  const handleEdit = (data) => navigate(`/keuangan/tarif-spp/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/keuangan/tarif-spp/${data.id}`)

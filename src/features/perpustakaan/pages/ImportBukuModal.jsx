...
const downloadTemplate = () => {
  const headers = ['isbn', 'judul', 'penulis', 'penerbit', 'tahun', 'stok']
  const example = ['9786020000001', 'Buku Contoh — hapus baris ini', 'Penulis Contoh', 'Penerbit Contoh', '2026', '10']
  const guide = [
    ['Versi template', 'template_import_buku_v1.xlsx'],
    ['Sheet wajib', 'Data — row 1 header, row 2 contoh (hapus sebelum import)'],
    ['Wajib diisi', 'isbn, judul, stok'],
    ['isbn', 'Identifier unik, maksimal 20 karakter. Format sel sebagai teks.'],
    ['judul', 'Maksimal 150 karakter.'],
    ['penulis / penerbit', 'Opsional, maksimal 100 karakter.'],
    ['tahun', 'Opsional, integer 1900 sampai tahun depan.'],
    ['stok', 'Integer non-negatif. Import tidak pernah menambah stok buku existing.'],
    ['Duplicate', 'ISBN yang sudah terdaftar akan gagal (create-only).'],
  ...
  XLSX.writeFile(wb, 'template_import_buku_v1.xlsx')
}

export const ImportBukuModal = ({ onClose, onSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    const { data, error } = await bukuService.importExcel(file)
    setLoading(false)

    if (error) {
      showError(error?.message || 'Gagal mengimpor data buku.')
      return
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="import-buku-title"
        onKeyDown={handleDialogKeyDown}
        className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg overflow-hidden"
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 id="import-buku-title" className="text-lg font-semibold text-gray-900 dark:text-white">Import Data Buku</h2>
          <button
            ref={initialFocusRef}
            onClick={onClose}
            disabled={loading}
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left font-medium text-gray-600 dark:text-gray-300">ISBN</th>
...
export default ImportBukuModal

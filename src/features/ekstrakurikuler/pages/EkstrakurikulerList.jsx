...
const EkstrakurikulerList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await ekstrakurikulerService.getAll(params)
  ...
  const handleEdit = (data) => navigate(`/ekstrakurikuler/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/ekstrakurikuler/${data.id}`)

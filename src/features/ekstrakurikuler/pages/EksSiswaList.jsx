...
const EksSiswaList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await eksSiswaService.getAll(params)
    ...
    if (data) {
      ...
      console.error('Error fetching ekstrakurikuler siswa:', error)
      showError('Gagal mengambil data pendaftaran ekskul')
    }
  ...
  const handleEdit = (data) => navigate(`/ekstrakurikuler/siswa/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/ekstrakurikuler/siswa/${data.id}`)

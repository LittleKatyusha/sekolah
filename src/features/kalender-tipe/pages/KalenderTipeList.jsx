...
const KalenderTipeList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await kalenderTipeService.getAll(params)
    if (data) {
      setRowData(data.data || [])
      if (data.meta) { setTotalRows(data.meta.total || 0); setCurrentPage(data.meta.current_page || page) }
    } else {
      showError('Gagal mengambil data kalender tipe')
    }
    setLoading(false)
  }, [pageSize, searchText])
  ...
  const handleDelete = async (data) => {
    ...
    if (result.isConfirmed) {
      ...
      if (!error) { showSuccess(`${data.nama} berhasil dihapus!`); fetchData(currentPage, pageSize) }
      else showError('Gagal menghapus kalender tipe')
    }
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            ...
            <input type="text" placeholder="Cari tipe..." value={searchText} onChange={(e) => setSearchText(e.target.value)}

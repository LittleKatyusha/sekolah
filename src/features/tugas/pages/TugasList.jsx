...
const TugasList = () => {
  ...
  const fetchTugas = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await tugasService.getAll(params)
  ...
  useEffect(() => {
    ...
    fetchTugas(1, pageSize, searchText)
  }, [])

  const handleEdit = (data) => {
    navigate(`/akademik/tugas/${data.id}/edit`)
  }

  const handleDetail = (data) => {
    navigate(`/akademik/tugas/${data.id}`)
  }

  const handleDelete = async (data) => {
    const label = `Tugas "${data.judul || ''}"`
    const result = await showDeleteConfirm(label)
    if (result.isConfirmed) {
      const { error } = await tugasService.delete(data.id)
      if (!error) {
        showSuccess(`${label} berhasil dihapus!`)
        fetchTugas(currentPageRef.current, pageSize, searchText)
      } else {
  ...
  const onPaginationChanged = useCallback((params) => {
    ...
    if (newPageSize !== pageSize) {
      ...
      pageCursorsRef.current = { 1: null }
      currentPageRef.current = 1
      fetchTugas(1, newPageSize, searchText)
      return
    }
    
    // Handle page number change
    if (newPageNumber !== currentPageRef.current) {
      fetchTugas(newPageNumber, pageSize, searchText)
    }
  }, [pageSize, searchText, fetchTugas])
  ...
  const onFilterTextBoxChanged = useCallback((e) => {
    ...
    fetchTugas(1, pageSize, value)
  }, [fetchTugas, pageSize])

  // Handle refresh
  const handleRefresh = useCallback(() => {
    fetchTugas(currentPageRef.current, pageSize, searchText)
  }, [fetchTugas, pageSize, searchText])
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <Button onClick={() => navigate('/akademik/tugas/create')}>

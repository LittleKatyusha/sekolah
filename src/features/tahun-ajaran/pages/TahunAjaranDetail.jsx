...
import { ArrowLeft, Edit, Trash2, Calendar, Hash, CheckCircle } from 'lucide-react'
...
const TahunAjaranDetail = () => {
  ...
  const handleDelete = async () => {
    const label = `Tahun Ajaran "${tahunAjaran.nama || ''}"`
    const result = await showDeleteConfirm(label)
    if (result.isConfirmed) {
      const { error } = await tahunAjaranService.delete(tahunAjaran.id)
      if (!error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Detail Tahun Ajaran</h1>
        </div>
        <div className="flex gap-3">
          {can('tahun-ajaran.update') && (
            <Button variant="warning" onClick={() => navigate(`/admin/tahun-ajaran/${id}/edit`)}>
          ...
          <Card>
            <div className="p-6 text-center">
              ...
              <div className="flex justify-center gap-2 mb-2">
                {getStatusBadge(tahunAjaran.is_active)}
              </div>

              <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4 text-left space-y-3">
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">ID</span>
                  <span className="font-medium text-gray-900 dark:text-white">{tahunAjaran.id}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500 dark:text-gray-400">Kode</span>
                  <span className="font-medium text-gray-900 dark:text-white">{tahunAjaran.kode || '-'}</span>
                </div>
              </div>
            </div>
          </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{tahunAjaran.kode || '-'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{tahunAjaran.nama || '-'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{formatDate(tahunAjaran.tanggal_mulai)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-white">{formatDate(tahunAjaran.tanggal_selesai)}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="mt-1">{getStatusBadge(tahunAjaran.is_active)}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="text-gray-700 dark:text-gray-300">{formatDate(tahunAjaran.created_at)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Terakhir diperbarui</p>
                    <p className="text-gray-700 dark:text-gray-300">{formatDate(tahunAjaran.updated_at)}</p>
                  </div>
                </div>
              </div>
            </div>
...
export default TahunAjaranDetail
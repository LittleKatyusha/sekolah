...
const AnggotaList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await anggotaService.getAll(params)
    ...
    if (data) {
      ...
      console.error('Error fetching anggota organisasi:', error)
      showError('Gagal mengambil data anggota organisasi')
    }
  ...
  const handleEdit = (data) => navigate(`/organisasi/anggota/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/organisasi/anggota/${data.id}`)

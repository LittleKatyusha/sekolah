...
const OrganisasiList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = pageSize, searchQuery = searchText) => {
    ...
    const { data, error } = await organisasiService.getAll(params)
  ...
  const handleEdit = (data) => navigate(`/organisasi/${data.id}/edit`)
  const handleDetail = (data) => navigate(`/organisasi/${data.id}`)

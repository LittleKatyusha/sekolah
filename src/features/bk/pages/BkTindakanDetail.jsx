...
import { ArrowLeft, Edit, Trash2, Calendar, Briefcase, ClipboardList } from 'lucide-react'
...
const BkTindakanDetail = () => {
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      const { error } = await bkTindakanService.delete(tindakan.id)
      if (!error) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Detail Tindakan BK</h1>
        </div>
        <div className="flex gap-3">
          {can('bk-tindakan.update') && (
            <Button variant="warning" onClick={() => navigate(`/bk/tindakan/${id}/edit`)}>
              <Edit size={18} className="mr-2" />
              Edit
            </Button>
          )}
          {can('bk-tindakan.delete') && (
      ...
      <Card>
        <div className="p-6">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Informasi Tindakan BK</h3>
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="font-medium text-gray-900 dark:text-white">Kasus #{tindakan.trx_bk_kasus_id}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400">Deskripsi Tindakan</p>
                <p className="font-medium text-gray-900 dark:text-white whitespace-pre-wrap">{tindakan.deskripsi_tindakan || '-'}</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">{formatDateTime(tindakan.created_at)}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900 dark:text-white">{formatDateTime(tindakan.updated_at)}</p>
                </div>
              </div>
            </div>
          </div>
...
export default BkTindakanDetail
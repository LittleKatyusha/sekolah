...
const BkJenisList = () => {
  ...
  const fetchData = useCallback(async (page = 1, perPage = 10, search = '') => {
    ...
    const { data, error } = await bkJenisService.getAll(params)
    if (data) {
      ...
      console.error('Error fetching jenis BK:', error)
  ...
  const handleEdit = (data) => {
    navigate(`/bk/jenis/${data.id}/edit`)
  }

  const handleDetail = (data) => {
    navigate(`/bk/jenis/${data.id}`)
  }

  const handleDelete = async (data) => {
    const result = await showDeleteConfirm(data.nama)
    if (result.isConfirmed) {
      const { error } = await bkJenisService.delete(data.id)
      if (!error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Data Jenis BK</h1>
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
            <input
              type="text"
              placeholder="Cari jenis BK..."
              value={searchText}

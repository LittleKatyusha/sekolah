...
import { z } from 'zod'
import { UserPlus, Mail, Lock, Eye, EyeOff, Shield, User } from 'lucide-react'
...
const registerSchema = z.object({
  ...
  role: z.enum(['guru', 'siswa', 'bk', 'orang_tua'], {
...
const Register = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const { data: response, error: apiError } = await apiService.post('/auth/register', {
        name: data.name,
        email: data.email,
      ...
      showSuccess('Account created successfully! Please login.', 'Registered!')
      setLoading(false)
      navigate('/login')
    } catch (err) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <div>
              ...
              <select
                ...
                } rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200`}
              >
